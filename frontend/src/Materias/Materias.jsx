...
const Materias = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/materias/${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
        // Refrescar la lista desde el backend para asegurar que la materia se eliminó en la base de datos
        const materiasRes = await fetch(`${API_BASE_URL}/materias`);
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <main className="flex-1 flex flex-col items-center py-8 overflow-auto bg-gray-100 text-gray-900">
          <div className="w-full max-w-5xl">
            ...
            <div className="bg-white border border-gray-200 rounded-b-md p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="flex-1 flex flex-col md:flex-row md:items-center gap-2">
                ...
                <div className="relative flex-1">
                  ...
                  <input id="materia-search" type="text" placeholder="Nombre de la Materia" className="pl-10 pr-3 py-2 border border-gray-300 bg-white text-gray-900 rounded-md w-full focus:outline-none focus:ring-2 focus:ring-blue-400" value={search} onChange={e => setSearch(e.target.value)} />
                </div>
              </div>
              <div className="flex flex-col md:flex-row md:items-center gap-2 min-w-fit">
                ...
                <select id="ordenar" className="border border-gray-300 bg-white text-gray-900 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" value={sortOrder} onChange={e => setSortOrder(e.target.value)}>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-white">
                    <tr>
                      ...
                      <th className="px-2 py-2 text-center text-xs font-bold text-gray-700 uppercase">Facultad</th>
                      <th className="px-2 py-2 text-center text-xs font-bold text-gray-700 uppercase">Carrera</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-100">
                    {filteredMaterias.length === 0 ? (
                      <tr><td colSpan={12} className="text-center py-4 text-gray-500">No hay materias registradas.</td></tr>
                    ) : (
                      filteredMaterias.map((materia, idx) => (
                        <tr key={materia.materia_id || idx} className="hover:bg-blue-50 transition-colors">
                          <td className="px-2 py-2 whitespace-nowrap text-sm text-gray-700 font-semibold">
                            <button className="text-red-600 hover:text-red-800 focus:outline-none" title="Eliminar" onClick={() => handleDelete(materia.materia_id)}>
                              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          ...
                          <td className="px-2 py-2 whitespace-nowrap text-sm text-blue-800 font-bold">{materia.nombre}</td>
