...
const RegistroAcademias = () => {
  ...
  const handleSubmit = async e => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/academias`, {
        method: "POST",
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <main className="flex-1 flex flex-col items-center py-8 overflow-auto">
          <div className="w-full max-w-5xl">
            <div className="bg-[#3578b3] text-white text-lg font-semibold rounded-t-md px-4 py-2 text-center mb-2">Academia</div>
            <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-b-md p-6 flex flex-col gap-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                <div>
                  ...
                  <input type="text" name="nombre" value={form.nombre} onChange={handleChange} className="w-full border rounded px-3 py-2 bg-white border-gray-300 text-gray-900" required />
                </div>
                <div>
                  <label className="block text-gray-800 font-bold mb-1">Facultad:</label>
                  <select
                    name="facultad_id"
              ...
              <div className="flex flex-col items-center mt-2 gap-2">
                <button type="submit" disabled={loading} className="bg-[#3578b3] text-white font-semibold w-1/2 md:w-1/3 px-12 py-2 rounded hover:bg-[#285a87] transition-colors disabled:opacity-60 disabled:cursor-not-allowed">
